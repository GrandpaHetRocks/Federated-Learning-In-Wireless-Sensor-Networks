# -*- coding: utf-8 -*-
"""
Created on Sun Oct  3 13:02:22 2021
...
from N_cluster_snr import cluster_former
import copy
...
#     active_clients = [clients[i] for i in active_clients_inds]
    ...
    members3=arranged_clusters[2]['Members']
    print(len(members1))
    print(len(members2))
    print(len(members3))
    new_members1=[]
    ...
    arranged_clusters[2]['Members']=new_members3
    ...
    cl_no=int(arranged_clusters[2]['Cluster Head'][6:])
    arranged_clusters[2]['Cluster Head']=clients[cl_no-1]
    ...
    for k in snrlist:
        if(arranged_clusters[0]["Cluster Head"]['hook'].id in k):
            snrsum1+=k[2]
        elif(arranged_clusters[2]["Cluster Head"]['hook'].id in k):
            snrsum3+=k[2]
        else:
            snrsum2+=k[2]
    ...
    weight1=snrsum1/(snrsum1+snrsum2+snrsum3)
    weight2=snrsum2/(snrsum1+snrsum2+snrsum3)
    weight3=snrsum3/(snrsum1+snrsum2+snrsum3)
    snrplot.append((snrsum1+snrsum2+snrsum3)/len(snrlist))
    
    head1=arranged_clusters[0]["Cluster Head"]
    head2=arranged_clusters[1]["Cluster Head"]
    head3=arranged_clusters[2]["Cluster Head"]
    
    overall=averageModelscluster(overall,[head1,head2,head3],[weight1,weight2,weight3])
