...
def load_dataset(num_users,iidtype):#this function helps load the datasets we made using mnistIID
    ...
    trainset=torchvision.datasets.FMNIST(root="./",train= False,transform=transform,download=True)
    testset=torchvision.datasets.FMNIST(root="./",train= False,transform=transform,download=True)
    train_group=None
    test_group=None
    if iidtype=='iid':
